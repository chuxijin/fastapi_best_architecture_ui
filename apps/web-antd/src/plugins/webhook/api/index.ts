...
export interface HeaderValidationRule {
  ...
  header_value: string;
  is_required: boolean;
  validation_type: 'exact' | 'contains' | 'regex';
}
...
export class WebhookApi {
   ...
   * 获取Webhook事件列表
   */
  async getWebhookList(params?: WebhookListParam & { page?: number; size?: number }) {
  ...
  async sendValidatedTestWebhook(
    ...
    return this.request.post<{
      ...
      headers: validationRules?.reduce((acc, rule) => {
   ...
   * 获取WebhookConfig列表
   */
  async getWebhookConfigList(params?: WebhookConfigListParam & { page?: number; size?: number }) {
    return this.request.get<{
      items: WebhookConfig[];
      total: number;
      page: number;
  ...
  async deleteWebhookConfigs(pks: number[]) {
    return this.request.delete('/api/v1/sys/webhook-configs', { data: { pks } });
  }
  ...
  async updateWebhookConfigStatus(id: number, is_active: boolean) {
    return this.request.put(`/api/v1/sys/webhook-configs/${id}/status?is_active=${is_active}`, {});
  }
  ...
  async getActiveWebhookConfigs() {
    return this.request.get<WebhookConfig[]>('/api/v1/sys/webhook-configs/active');
  }
}
...
export const retryFailedWebhooksApi = webhookApi.retryFailedWebhooks.bind(webhookApi);
export const getPendingWebhooksApi = webhookApi.getPendingWebhooks.bind(webhookApi);
export const sendTestWebhookApi = webhookApi.sendTestWebhook.bind(webhookApi);
export const sendValidatedTestWebhookApi = webhookApi.sendValidatedTestWebhook.bind(webhookApi);

// 导出WebhookConfig API函数
export const getWebhookConfigListApi = webhookApi.getWebhookConfigList.bind(webhookApi);
export const getWebhookConfigDetailApi = webhookApi.getWebhookConfigDetail.bind(webhookApi);
export const createWebhookConfigApi = webhookApi.createWebhookConfig.bind(webhookApi);
export const updateWebhookConfigApi = webhookApi.updateWebhookConfig.bind(webhookApi);
export const deleteWebhookConfigsApi = webhookApi.deleteWebhookConfigs.bind(webhookApi);
export const updateWebhookConfigStatusApi = webhookApi.updateWebhookConfigStatus.bind(webhookApi);
export const getActiveWebhookConfigsApi = webhookApi.getActiveWebhookConfigs.bind(webhookApi);
