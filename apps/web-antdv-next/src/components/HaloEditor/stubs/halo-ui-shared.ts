...
export const utils = {
  ...
  attachment: {
    ...
    getUrl: (attachment: AttachmentLike): string | undefined => {
      ...
      if (!attachment) return undefined;
      return (
        attachment.status?.permalink || attachment.url || attachment.permalink
      );
    },
  },
};
export const stores = {
  globalInfo: () => ({ globalInfo: { externalUrl: '' } }),
};
export const Dialog = {
  info: (opts: any) => {
    if (window.confirm(opts.description)) opts.onConfirm?.();
  },
};
