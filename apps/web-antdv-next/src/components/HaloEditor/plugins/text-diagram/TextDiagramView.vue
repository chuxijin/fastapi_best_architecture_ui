...
function onEditorChange(event: Event) {
  ...
  props.updateAttributes({ content: target.value });
}
</script>
<template>
  <node-view-wrapper
  ...
  >
    ...
    <div class="text-diagram-editor-panel">
      <div class="text-diagram-code relative overflow-hidden">
        <textarea
          :value="node.attrs.content"
          @input="onEditorChange"
          class="absolute inset-0 w-full h-full p-2 resize-none outline-none border-none bg-gray-50 font-mono text-sm"
          placeholder="请输入 Mermaid 或 PlantUML 代码..."
        ></textarea>
      </div>
      <div
        ref="previewRef"
