...
const MarkdownEdited = Node.create<ExtensionOptions>({
  ...
  addNodeView() {
    return VueNodeViewRenderer(CodeMirrorView as any);
  },
    ...
    return [
      {
        ...
        getContent: (node, schema) => {
          ...
          let markdown = htmlNode.textContent || htmlNode.innerHTML;
          if (!markdown) {
            markdown = "<br>";
          }
  ...
  renderHTML({ HTMLAttributes, node }) {
    ...
    container.innerHTML = marked.parse(content.firstChild.text || "") as string;
    return {
      dom: container,
    };
  },
