...
<template>
  <node-view-wrapper
    class="mt-3 overflow-hidden rounded outline outline-1 outline-[#ccc] transition-all hover:outline-[#55c6a0]"
  >
    <div
      class="flex items-center justify-between border-b border-[#e4e7ed] bg-[#f5f7fa] px-3 py-2"
      :class="{ 'border-b-0': collapsed }"
    >
      <div class="flex items-center gap-2" @click.self="collapsed ? (collapsed = false) : null">
        <button
          class="flex h-8 w-8 cursor-pointer items-center justify-center rounded border-none bg-transparent p-0 text-[#606266] transition-all duration-200 hover:bg-black/6 active:bg-black/10"
        ...
        <span class="text-sm font-medium text-[#606266]">{{ blockLabel }} 编辑块</span>
      </div>
    ...
    <div
      ...
      :class="[
        isSplitMode
          ? 'flex min-h-[400px] flex-row gap-[1px] bg-[#e4e7ed]'
          : '',
        isPreviewMode && !isSplitMode ? 'p-3' : '',
      ]"
    ...
    >
      <template v-if="isSplitMode">
        <div
          ...
          class="flex min-w-0 flex-1 flex-col overflow-auto bg-white"
        ></div>
        <div
          ref="previewContainerRef"
          class="min-w-0 flex-1 overflow-auto bg-white p-3 [&_.html-edited]:pointer-events-none [&_.markdown-edited]:pointer-events-none"
        ></div>
      </template>
