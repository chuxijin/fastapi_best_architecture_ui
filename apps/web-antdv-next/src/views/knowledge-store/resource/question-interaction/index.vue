...
import type {
  InteractionMaterialBlocksResult,
  InteractionMaterialQuestionResult,
  MaterialAnchorParams,
  MaterialAnchorQueryParams,
  MaterialAnchorResult,
  MaterialAnchorSource,
  MaterialAnchorType,
  QuestionDetail,
  QuestionInteractionAnnotationParams,
  QuestionInteractionAnnotationQueryParams,
  QuestionInteractionAnnotationResult,
  QuestionInteractionSelectionMode,
} from '#/api';
...
import {
  createMaterialAnchorsBatchApi,
  createQuestionInteractionAnnotationApi,
  deleteMaterialAnchorsApi,
  deleteQuestionInteractionAnnotationsApi,
  getInteractionMaterialBlocksApi,
  getInteractionMaterialQuestionsApi,
  getMaterialAnchorListApi,
  getMaterialListApi,
  getQuestionDetailApi,
  getQuestionInteractionAnnotationListApi,
  updateMaterialAnchorApi,
  updateQuestionInteractionAnnotationApi,
} from '#/api';
...
interface AnchorImagePointerState {
  ...
  currentX: number;
  currentY: number;
  naturalHeight?: null | number;
  naturalWidth?: null | number;
  startX: number;
  startY: number;
}
...
const anchorMaterialPreview = ref<InteractionMaterialBlocksResult | null>(null);
const anchorMaterialAnchors = ref<MaterialAnchorResult[]>([]);
...
const anchorGridOptions: VxeTableGridOptions<MaterialAnchorResult> = {
  rowConfig: {
    keyField: 'id',
    isHover: true,
  },
  ...
  proxyConfig: {
    ajax: {
      query: async (_, formValues) => {
        return await getMaterialAnchorListApi(
          cleanParams(formValues) as MaterialAnchorQueryParams,
        );
      },
    },
  },
...
const annotationGridOptions: VxeTableGridOptions<QuestionInteractionAnnotationResult> =
  {
    rowConfig: {
      keyField: 'id',
      isHover: true,
    ...
    columns: [
      ...
      { field: 'material_id', title: '材料 ID', width: 110 },
      { field: 'annotation_key', title: '标注键', minWidth: 180 },
      {
      ...
      },
      {
        field: 'is_default',
        title: '默认',
        slots: { default: 'is_default_default' },
        width: 90,
      },
      {
        field: 'status',
      ...
      ajax: {
        query: async (_, formValues) => {
          const params = cleanParams({
            ...formValues,
            is_default:
              formValues.is_default === undefined ||
              formValues.is_default === null ||
              formValues.is_default === ''
                ? undefined
                : formValues.is_default === 'true',
          });
          return await getQuestionInteractionAnnotationListApi(
            params as QuestionInteractionAnnotationQueryParams,
          );
        },
      },
    },
...
function openEditAnchor(row: unknown) {
  void loadMaterialOptions();
  resetAnchorForm(row as MaterialAnchorResult);
  anchorMaterialPreview.value = null;
  anchorMaterialAnchors.value = [];
  anchorDrafts.value = [];
  anchorDraftSequence.value = 0;
...
function openEditAnnotation(row: unknown) {
  void loadMaterialOptions();
  resetAnnotationForm(row as QuestionInteractionAnnotationResult);
  annotationMaterialAnchors.value = [];
  annotationMaterialPreview.value = null;
  annotationMaterialQuestions.value = [];
  annotationQuestionPreview.value = null;
  ...
  annotationRoleTargetId.value = null;
  annotationDrawerOpen.value = true;
  loadAnnotationQuestionPreview();
  loadAnnotationMaterialBlocks();
}

function resetAnchorForm(row?: MaterialAnchorResult) {
  ...
  Object.assign(anchorForm, {
    ...
    asset_hash: row.asset_hash ?? null,
    ...
    natural_height: row.natural_height ?? null,
    natural_width: row.natural_width ?? null,
    ocr_confidence:
...
function resetAnnotationForm(row?: QuestionInteractionAnnotationResult) {
  ...
  Object.assign(annotationForm, {
    ...
    answer_data_json: toPrettyJson(row.answer_data ?? {}),
    candidate_anchor_ids_text: row.candidate_anchor_ids?.join(', ') || '',
    config_json: toPrettyJson(row.config ?? {}),
    content_hash: row.content_hash ?? null,
    id: row.id,
...
async function submitAnchor() {
  ...
  try {
    if (anchorForm.id) {
      ...
      await updateMaterialAnchorApi(anchorForm.id, updatePayload);
      ...
      await createMaterialAnchorsBatchApi({
        items: anchorDrafts.value.map((item) => item.payload),
      });
...
async function submitAnnotation() {
  let payload: QuestionInteractionAnnotationParams;
  try {
  ...
  try {
    if (annotationForm.id) {
      const { question_id: _, ...updatePayload } = payload;
      await updateQuestionInteractionAnnotationApi(
        annotationForm.id,
        updatePayload,
      );
      message.success('题目交互标注已更新');
    } else {
      await createQuestionInteractionAnnotationApi(payload);
...
function confirmDeleteAnchor(row: unknown) {
  const anchor = row as MaterialAnchorResult;
  Modal.confirm({
    title: '确认删除材料锚点？',
    ...
    okText: '删除',
    async onOk() {
      await deleteMaterialAnchorsApi([anchor.id]);
...
function confirmDeleteAnnotation(row: unknown) {
  const annotation = row as QuestionInteractionAnnotationResult;
  Modal.confirm({
    title: '确认删除题目交互标注？',
    content: `标注：${annotation.annotation_key}（ID: ${annotation.id}）`,
    okButtonProps: { danger: true },
    okText: '删除',
    async onOk() {
      await deleteQuestionInteractionAnnotationsApi([annotation.id]);
...
async function fillCandidateAnchorsByMaterial() {
  ...
  try {
    const data = await getMaterialAnchorListApi({
      material_id: annotationForm.material_id,
      status: 10,
    });
...
async function loadAnchorMaterialBlocks() {
  ...
  try {
    const data = await getInteractionMaterialBlocksApi(anchorForm.material_id);
...
async function loadAnchorAnchorsByMaterial(showSuccess = true) {
  ...
  const data = await getMaterialAnchorListApi({
    material_id: anchorForm.material_id,
  });
...
async function loadAnnotationQuestionPreview() {
  ...
  try {
    const data = await getQuestionDetailApi(annotationForm.question_id);
...
async function loadAnnotationMaterialBlocks() {
  ...
  try {
    const data = await getInteractionMaterialBlocksApi(
      annotationForm.material_id,
    );
    annotationMaterialPreview.value = data;
    annotationForm.content_hash = data.content_hash;
    await Promise.all([
      loadAnnotationAnchorsByMaterial(false),
      loadAnnotationMaterialQuestions(false),
    ]);
    message.success(
      `已载入 ${data.blocks.length} 个材料块、${annotationMaterialQuestions.value.length} 道关联题目`,
    );
  } catch (error) {
    showErrorMessage(error, '载入材料内容失败');
  } finally {
...
async function loadAnnotationMaterialQuestions(showSuccess = true) {
  if (!annotationForm.material_id) {
    message.warning('请先选择材料');
    return;
  }
  ...
  try {
    const data = await getInteractionMaterialQuestionsApi(
      annotationForm.material_id,
    );
    annotationMaterialQuestions.value = data;
    if (showSuccess) {
      message.success(`已载入 ${data.length} 道关联题目`);
    }
  } catch (error) {
    showErrorMessage(error, '载入关联题目失败');
  }
}

async function loadAnnotationAnchorsByMaterial(showSuccess = true) {
  if (!annotationForm.material_id) {
    message.warning('请先选择材料');
    return;
  }
...
function buildAnchorPayload(): MaterialAnchorParams {
  ...
  const payload = stripUndefined({
    ...
    asset_hash: normalizeString(anchorForm.asset_hash),
    asset_url: normalizeString(anchorForm.asset_url),
    ...
    material_id: Number(anchorForm.material_id),
    natural_height: normalizeInteger(anchorForm.natural_height),
    natural_width: normalizeInteger(anchorForm.natural_width),
    ocr_confidence: normalizeNumber(anchorForm.ocr_confidence),
    ...
    table_cell: parseOptionalJsonObject(
      anchorForm.table_cell_json,
      '表格单元格 JSON',
    ),
    text: normalizeString(anchorForm.text),
  }) as unknown as MaterialAnchorParams;
...
function resetAnchorDraftAfterAdd(payload: MaterialAnchorParams) {
...
function selectAnchorMaterialBlock(block: Record<string, any>) {
  ...
  if (isImageBlock(block)) {
    ...
    anchorForm.asset_url = normalizeString(
      block.asset_url || block.image_url || block.url || block.src,
    );
    anchorForm.natural_width = normalizeInteger(block.natural_width);
    anchorForm.natural_height = normalizeInteger(block.natural_height);
...
function beginAnchorImageSelection(
  ...
  anchorImagePointerState.value = {
    ...
    currentX: pointer.x,
    currentY: pointer.y,
    naturalHeight: normalizeInteger(block.natural_height),
    naturalWidth: normalizeInteger(block.natural_width),
    startX: pointer.x,
    startY: pointer.y,
  };
}
...
function finishAnchorImageSelection(
  ...
  anchorForm.block_id = blockId;
  anchorForm.asset_url = start.assetUrl;
  anchorForm.natural_width = start.naturalWidth;
  anchorForm.natural_height = start.naturalHeight;
...
function toggleAnnotationCandidateAnchor(anchor: MaterialAnchorResult) {
...
function setAnnotationAnswerAnchor(anchor: MaterialAnchorResult) {
  const ids = getAnnotationCandidateIds();
  if (!ids.includes(anchor.id)) {
    setAnnotationCandidateIds([...ids, anchor.id]);
  }
...
function isAnnotationCandidateAnchor(anchor: MaterialAnchorResult) {
  return getAnnotationCandidateIds().includes(anchor.id);
}

function isAnnotationAnswerAnchor(anchor: MaterialAnchorResult) {
...
function selectAnnotationAnchorRole(anchor: MaterialAnchorResult) {
  annotationRoleTargetId.value = anchor.id;
  annotationFocusedAnchorId.value = anchor.id;
  if (!isAnnotationCandidateAnchor(anchor)) {
    setAnnotationCandidateIds([...getAnnotationCandidateIds(), anchor.id]);
  }
}
...
function buildAnnotationAnswerData(anchor: MaterialAnchorResult) {
...
function buildAnnotationPayload(): QuestionInteractionAnnotationParams {
  fillAnnotationAutoFields();
  ...
  if (candidateAnchorIds.length === 0) {
  ...
  return stripUndefined({
    annotation_key: annotationForm.annotation_key.trim(),
    answer_data: answerData,
    candidate_anchor_ids: candidateAnchorIds,
    config: parseJsonObject(annotationForm.config_json, '配置 JSON'),
    content_hash: normalizeString(annotationForm.content_hash),
    instruction: annotationForm.instruction.trim(),
    interaction_type: annotationForm.interaction_type.trim(),
    is_default: annotationForm.is_default,
    material_id: normalizeNumber(annotationForm.material_id),
    question_id: Number(annotationForm.question_id),
    selection_mode: annotationForm.selection_mode,
    status: annotationForm.status,
    title: normalizeString(annotationForm.title),
    version_no: Number(annotationForm.version_no || 1),
  }) as unknown as QuestionInteractionAnnotationParams;
}
...
function getAnnotationTextBlockSegments(block: Record<string, any>) {
  ...
  const segments: Array<{
    anchor: MaterialAnchorResult | null;
    endOffset: number;
    role: null | string;
    selected: boolean;
    startOffset: number;
...
function formatAnchorDisplayText(anchor: MaterialAnchorResult) {
...
function sanitizeAnchorPayload(payload: MaterialAnchorParams) {
...
function clearImageAnchorFields() {
  anchorForm.asset_hash = null;
  anchorForm.asset_url = null;
  anchorForm.bbox_json = '';
  anchorForm.natural_height = null;
  anchorForm.natural_width = null;
  anchorForm.ocr_confidence = null;
  anchorForm.polygon_json = '';
}

function formatAnchorPosition(anchor: MaterialAnchorResult) {
...
<template>
  <Page auto-content-height content-class="flex h-full min-h-0 flex-col">
    ...
    <a-tabs v-model:active-key="activeTab" class="interaction-tabs">
      <a-tab-pane key="anchors" tab="材料锚点">
        <AnchorGrid class="h-full">
          <template #toolbar-actions>
            <VbenButton @click="openCreateAnchor">
              <MaterialSymbolsAdd class="size-5" />
              新增锚点
            </VbenButton>
          </template>

          <template #anchor_type_default="{ row }">
            <a-tag color="blue">{{ row.anchor_type }}</a-tag>
          </template>

          <template #status_default="{ row }">
            <a-tag :color="getStatusMeta(row.status).color">
              {{ getStatusMeta(row.status).label }}
            </a-tag>
          </template>

          <template #anchor_operation_default="{ row }">
            <a-space>
              <a-button size="small" type="link" @click="openEditAnchor(row)">
                编辑
              </a-button>
              <a-button
                danger
      ...
      <a-tab-pane key="annotations" tab="题目交互标注">
        <AnnotationGrid class="h-full">
          ...
          <template #candidate_anchor_ids_default="{ row }">
            {{ row.candidate_anchor_ids?.length || 0 }} 个
          </template>

          <template #is_default_default="{ row }">
            <a-tag :color="row.is_default ? 'green' : 'default'">
              {{ row.is_default ? '默认' : '否' }}
            </a-tag>
          </template>
      ...
      <a-form layout="vertical" :model="annotationForm">
        ...
        >
          <div class="mb-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
            <span>题型：{{ annotationQuestionPreview.type }}</span>
            <span>难度：{{ annotationQuestionPreview.difficulty }}</span>
            <span>
              材料：
              {{
                annotationQuestionPreview.material_ids?.length
                  ? annotationQuestionPreview.material_ids.join(', ')
                  : '无'
              }}
            </span>
          </div>
        ...
        <a-form-item label="已选候选锚点">
          <div class="rounded border bg-gray-50 p-3">
            ...
            <a-empty v-else class="mb-3" description="还没有选择候选锚点" />
            <a-button
