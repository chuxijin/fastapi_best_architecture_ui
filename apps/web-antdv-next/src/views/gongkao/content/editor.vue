...
async function handleSave(publishNow: boolean, goBack: boolean = true) {
  ...
  if (!formData.value.slug?.trim()) {
    return message.warning('请填写 slug');
  }
...
function removeTag(tag: string) {
  formData.value.tags =
    formData.value.tags?.filter((item: string) => item !== tag) || [];
}

function handleGenerateSlug() {
  const charset = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  const len = 16;
  formData.value.slug = Array.from(
    new Uint32Array(len),
        ...
        <AForm layout="vertical">
          ...
          <ACard size="small" title="目录与大纲" class="sidebar-card">
            ...
            <div class="mt-4 border-t pt-3" v-if="tocItems.length > 0">
              ...
              <ul
                class="m-0 p-0 list-none text-sm max-h-[300px] overflow-y-auto"
              >
                <li
                ...
                >
                  <span class="opacity-40 mr-1 text-xs">H{{ item.level }}</span>
                  {{ item.title }}
                </li>
              </ul>
            </div>
            <div
              v-else
              class="mt-4 pt-3 border-t text-xs text-center text-gray-400"
            >
              编辑器内暂无标题
            </div>
          </ACard>
